        ...
        >
            {
                projects.map((item, index) => {
                    return <Project key={index} imageUrl={item.imageUrl} videoUrl={item.videoUrl} title={item.title}
                                    type={item.type} date={item.date} content={item.content} tags={item.tags}
                                    showMoreHeight={item.showMoreHeight} id={item.id}/>
                })
            }
        </div>
    );
            ...
            <div className="project-card">
                ...
                <div style={{paddingLeft: 10, paddingRight: 10}}>
                    ...
                    <div style={{display: "flex", flexDirection: "row", alignItems: "center"}}>
                        <p style={{color: "#0078ff"}}>{props.type}</p> <p
                        style={{marginLeft: 10, marginRight: 10, color: "rgba(50, 50, 50)"}}>/</p> <p
                        style={{color: "rgba(50, 50, 50)"}}>{props.date}</p>
                    </div>
                    <div ref={contentRef} style={{
                        marginBottom: "1rem",
                        overflow: "hidden",
                        maxHeight: showMore ? "999999px" : showMoreHeight
                    }}>
